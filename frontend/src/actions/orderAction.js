...
export const cancelMyOrder = (id) => async (dispatch) => {
  try {
    dispatch({ type: CANCEL_MY_ORDERS_REQUEST });
    const { data } = await axios.delete(`/api/v1/order/cancel/${id}`)
    dispatch({ type: CANCEL_MY_ORDERS_SUCCESS, payload: data });
  } catch (error) {
    dispatch({
      type: CANCEL_MY_ORDERS_FAIL,
      payload: error.response.data.message,
    });
  }
};
